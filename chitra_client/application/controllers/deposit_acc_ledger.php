...
Class Deposit_acc_ledger extends Controller {
    ...
    function index() {
        ...
        if ($this->session->userdata('logged_in_cum') == TRUE) {
            $this->display_data($dep_acc_no, $date_filter, $date_filter1);
            $this->load->view('vdeposit_acc_ledger', $this->data);
        } else {
    ...
    //----------------------------------------------------------------------
    function display_data($dep_acc_no, $date_filter, $date_filter1) {
        $this->check_login_status();
        if ($this->session->userdata('logged_in_cum') == TRUE) {
            $this->load->library('table');
            $tmpl = array('table_open' => '<table id="records">', 'heading_row_start' => '<thead><tr>', 'heading_row_end' => '</tr></thead><tbody>');
            $this->table->set_template($tmpl);
            $this->table->set_heading('ID', 'Tran_Date', 'Description', 'Deposit', 'Withdrawal', 'Balance');
            $from_date = date("Y-m-j", strtotime($date_filter));
            $to_date = date("Y-m-j", strtotime($date_filter1));
            $sql_deposit_ledger = "CALL sp_dep_acc_ledger('" . $dep_acc_no . "', '" . $from_date . "', '" . $to_date . "')";

            $query_deposit_ledger = $this->db->query($sql_deposit_ledger);
            $this->data['table_data'] = $this->table->generate($query_deposit_ledger);

            $this->db->close();
            $this->db->initialize();

            $sql = "SELECT t1.id, t1.name, t1.mobile_no FROM (SELECT id, name, mobile_no FROM tbl_client UNION SELECT id, name, mobile_no FROM tbl_member) t1 INNER JOIN tbl_deposit_acc t2 ON t2.client_id = t1.id WHERE t2.account_no ='" . $dep_acc_no . "'";
    ...
    //----------------------------------------------------------------------
    function filter_data($dep_acc_no, $date_filter, $date_filter1) {
        $this->check_login_status();
        $from_date = date("Y-m-j", strtotime($date_filter));
        $to_date = date("Y-m-j", strtotime($date_filter1));
        $sql_deposit_ledger = "CALL sp_dep_acc_ledger('" . $dep_acc_no . "', '" . $from_date . "', '" . $to_date . "')";
        $query_deposit_ledger = $this->db->query($sql_deposit_ledger);
        if ($query_deposit_ledger->num_rows() > 0) {
            echo json_encode($query_deposit_ledger->result());
        } else {
            echo json_encode(array());
        }
    }
    //----------------------------------------------------------------------
    //Functions for showing the Deposit Account No in the auto complete text box
    function get_dep_acc_no() {
        $this->check_login_status();

        if (isset($_REQUEST['q'])) {
            $sql = "SELECT t1.account_no as id, CONCAT(t1.account_no, '-', t2.name) AS name FROM tbl_deposit_acc t1 INNER JOIN (SELECT id, name FROM tbl_client UNION SELECT id, name FROM tbl_member) t2 ON t1.client_id = t2.id WHERE t1.account_no LIKE '%" . strip_slashes($_REQUEST['q']) . "%' OR t2.name LIKE '%" . strip_slashes($_REQUEST['q']) . "%' Order BY t1.open_date DESC";
    ...
    //----------------------------------------------------------------------
    //Print Deposit A/c Ledger in Pdf Format
    function print_pdf_ledger($date_filter, $date_filter1, $dep_acc_no) {
        $this->check_login_status();
        $from_date = date("Y-m-j", strtotime($date_filter));
        $to_date = date("Y-m-j", strtotime($date_filter1));
        ...
        $pdf->SetTitle('Deposit A/c Ledger');
        $pdf->SetSubject('Deposit A/c Ledger');
        $pdf->SetKeywords('Ledger, Deposit A/c Ledger');
...
//        $pdf->Image($this->config->item('base_url') . 'images/logo_ara.jpg', 50, 1, 17, 14);
        ...
        $pdf->CreateTextBox("Deposit A/c Ledger", 0, 25, 170, 10, 17, 'B', 'C');

        // create address box
        $sql_dep_acc = "SELECT t1.account_no, t2.name, t2.address_1 FROM tbl_deposit_acc t1 INNER JOIN (SELECT id, name, address_1 FROM tbl_client UNION SELECT id, name, address_1 FROM tbl_member) t2 ON t1.client_id = t2.id WHERE t1.account_no ='" . $dep_acc_no . "'";
        $qry_dep_acc = $this->db->query($sql_dep_acc);
        if ($qry_dep_acc->num_rows() > 0) {
            $result_dep_acc = $qry_dep_acc->row();
            $pdf->CreateTextBox(" ", 0, 30, 80, 10, 10);
            $pdf->CreateTextBox(" ", 0, 30, 80, 10, 10);
            $pdf->CreateTextBox("A/c: " . $result_dep_acc->account_no . "-". $result_dep_acc->name, 0, 30, 80, 10, 10);
            $pdf->CreateTextBox("Address: " . $result_dep_acc->address_1, 0, 35, 80, 10, 10);
            ...
            <table style="border-collapse:collapse;" cellspacing="0" cellpadding="1" border="1" nobr="true">
               <thead>
                   <tr>
                        ...
                        <th style="border-left-color:#000000;border-right-color:#000000;border-top-color:#000000;border-bottom-color:#000000;width:70px; text-align:center">Deposit</th>
...
EOD;
        ...
        $sql_dep_acc_ledger = "CALL sp_dep_acc_ledger('" . $dep_acc_no . "', '" . $from_date . "', '" . $to_date . "')";
        $qry_dep_acc_ledger = $this->db->query($sql_dep_acc_ledger);
        $arr_ledger = array();
        $arr_ledger_mod = array();
        $arr_tran = array();
        if ($qry_dep_acc_ledger->num_rows() > 0) {
            foreach ($qry_dep_acc_ledger->result() as $row) {
                $cur_id = $row->ID;
                $cur_tran_date = $row->Tran_Date;
                $cur_description = $row->Description;
                $cur_deposit = $row->Deposit;
                ...
                <tr>
                    ...
                    <td style="border-left-color:#000000;text-align:right;width:70px;">$cur_deposit</td>
...
EOD;
            ...
            $pdf->Output('dep_acc_ledger.pdf', 'I');
        }
    }
    //---------------------------------------------------------------
}
